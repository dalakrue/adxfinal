...
_PARTS = ['part_001', 'part_002', 'part_003', 'part_004', 'part_005', 'part_006']
_source = "".join(
    line
    for part_name in _PARTS
    for line in import_module(f"_home_joined_v9_parts.{part_name}").SOURCE_LINES
)
_ORIGINAL_FILE = str(Path(__file__).resolve().parent.parent / '_home_joined.py')
NAMESPACE = {
    "__name__": 'tabs._home_joined',
    "__package__": 'tabs',
