...
def render_cached_powerbi_projection(*, state: MutableMapping[str, Any] | None = None) -> None:
    ...
    try:
        ...
        if recovery_meta.get("ok"):
            ...
            # Always use candles generated from the same saved point path so a
            # future actual can never leak into the projection display.
    ...
    if integrity.get("valid"):
        ...
        st.error(f"Power BI projection state: {integrity.get('state')} — production chart blocked; no stale replacement was used.")
    ...
    if recovery_meta.get("ok"):
        ...
        st.caption(
            f"Saved-path source: {recovery_meta.get('source_provenance', 'canonical snapshot')} · "
            f"available horizons: {horizons} · interval status: {interval_status} · "
            "future actuals suppressed · prediction engine recalculated: NO"
        )
