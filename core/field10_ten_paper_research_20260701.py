...
This module opens those saved states only after the user selects Field 10,
calculates research diagnostics once per parent run/model version, and stores
read-only audit rows in the existing Field 10 SQLite database.
"""
...
def ensure_field10_research_validation(
    ...
    """Calculate the ten-paper layer only after Field 10 is explicitly opened."""
    ...
    report = {
        ...
        "version": VERSION, "lazy_trigger": "FIELD_10_SELECTED",
    }
    state[STATE_KEY] = report
    return report
