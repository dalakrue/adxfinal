...
def _state_label(side_frequency: float, other_frequency: float, current: str, side: str) -> str:
    aligned = side in str(current).upper()
    opposing = ("SELL" if side == "BUY" else "BUY") in str(current).upper()
    dominance = side_frequency - other_frequency
    if side_frequency >= 0.62 and dominance >= 0.12 and not opposing:
        return "Allowed"
    if aligned and side_frequency >= 0.40:
        return "Hold and Protect"
    if side_frequency >= 0.42 and dominance > -0.08:
