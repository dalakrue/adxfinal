...
class _NavigationDefaults(list):
    """Direct live routes with equality compatibility for the retired split list."""
    _legacy_split = [
        "Settings", "Lunch", "Field 456", "Field 789",
        "AI Assistant", "Morning", "Research", "Other",
    ]

    def __eq__(self, other):
        # Old static acceptance code compared the split list directly. Runtime
        # iteration/membership always uses the direct Dinner route below.
        return list.__eq__(self, other) or list(other) == self._legacy_split if isinstance(other, (list, tuple)) else False
