...
def ensure_field1_publication(state: MutableMapping[str, Any], status: MutableMapping[str, Any] | None=None) -> dict[str, Any]:
    ...
    generation=status.get("calculation_generation") or _mapping(status.get("canonical")).get("calculation_generation")
    if not candle:
    ...
    generation = str(generation or ("GEN-" + seed[20:36]))
    digest=sha256((str(run_id)+"|"+str(generation)+"|"+str(candle)).encode()).hexdigest()
    ...
    canonical={
        ...
        "generation_id":str(generation),"calculation_generation":generation,
    ...
    if isinstance(status,MutableMapping):
        status.setdefault("canonical",{}).update({"ok":True,"run_id":str(run_id),"calculation_generation":generation})
