...
def validate_canonical_payload(canonical: Mapping[str, Any], *, now: Any = None) -> ValidationReport:
    ...
    checks = [
        ...
        ConstraintResult("latest_completed_h1", "PASS" if latest_ts is not None and latest_ts <= now_ts.floor("h") else "FAIL", "CRITICAL", str(latest), f"<= {now_ts.floor('h').isoformat()}", 0 if latest_ts is not None and latest_ts <= now_ts.floor("h") else 1, "The canonical timestamp is a completed-H1 boundary already screened by the source gate; an exact hour boundary is valid."),
