...
def resolve_api_key(provider: str, state: Mapping[str, Any] | None = None) -> str:
    ...
    if provider in {"openrouter", "ai", "ai_api"}:
        return (
...
def secure_secret_status(state: Mapping[str, Any] | None = None) -> dict[str, Any]:
    ...
    openrouter_secret = bool(_secret_path("api_keys", "openrouter") or _secret_path("openrouter", "api_key") or os.getenv("OPENROUTER_API_KEY"))
    return {
        ...
        "openrouter_configured": bool(openrouter_secret or state.get("openrouter_api_key")),
        ...
        "openrouter_source": "Streamlit Secrets" if openrouter_secret else ("Temporary session replacement" if state.get("openrouter_api_key") else "Not configured"),
    }
