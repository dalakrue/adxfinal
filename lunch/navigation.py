...
def _initialize(options: list[str]) -> None:
    current = str(st.session_state.get(SELECTED_KEY) or options[0])
...
def render_field_navigation() -> tuple[str, bool]:
    ...
    if previous_col.button(
        ...
        st.session_state[SELECTED_KEY] = options[index - 1]
...
__all__ = ["render_field_navigation", "SELECTED_KEY", "OPEN_KEY"]
